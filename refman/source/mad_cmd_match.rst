...
**jacobian**
	A *callable* :literal:`(cres, fgrd, cjac, e)` that fills the objective gradient and/or the constraints Jacobian explicitly for the current command result :literal:`cres`. When provided, it supersedes the finite-difference Jacobian builder. (default: :const:`nil`).

	Example: :expr:`jacobian = my_jacobian`.
...
\
	...
	**ncond**
		A *number* specifying (:ref:`LSopt <sec.match.lsopt>` methods only) how many singular values must be discarded while solving the least-squares system with :literal:`ssolve`. A positive (resp. negative) integer discards the smallest (resp. largest) singular values. This attribute can be updated between iterations, e.g. through :literal:`env.objective.ncond`. (default: :const:`0`).

		Example: :expr:`ncond = 1`.
